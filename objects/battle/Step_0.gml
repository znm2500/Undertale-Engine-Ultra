if (heal != 0) {
    Player_SetHp(round(hp)) Player_SetKR(round(kr))
}
//菜单
if (!global.classic_ui){ 
    ...
    if (_menu == BATTLE_MENU.FIGHT_TARGET) {
        //上/下
        if (battle_soul.image_angle != 90) Anim_Create(battle_soul, "image_angle", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.image_angle, 90 - battle_soul.image_angle, 20) if (Input_IsPressed(INPUT.UP)) {
    ...
    if (_menu == BATTLE_MENU.ACT_TARGET) {
        ...
        //返回
        if (Input_IsPressed(INPUT.CANCEL)) {
            Anim_Create(battle_soul, "image_angle", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.image_angle, -battle_soul.image_angle, 20) Battle_SetMenu(BATTLE_MENU.BUTTON);
        }
        //确定
        if (Input_IsPressed(INPUT.CONFIRM)) {
            Anim_Destroy(battle_soul) audio_play_sound(snd_menu_confirm, 0, false);
    ...
    if (_menu == BATTLE_MENU.ACT_ACTION) {
        ...
        if (round(battle_soul.y) != ((((battle_board.y - battle_board.up) - 5) + 36) + (32 * floor((_menu_choice_action / 2)))) || round(battle_soul.x) != ((((battle_board.x - battle_board.left) - 5) + 40) + (256 * (_menu_choice_action % 2)))) {
            Anim_Create(battle_soul, "x", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.x, (((((battle_board.x - battle_board.left) - 5) + 40) + (256 * (_menu_choice_action % 2))) - battle_soul.x), 6) Anim_Create(battle_soul, "y", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.y, (((((battle_board.y - battle_board.up) - 5) + 36) + (32 * floor((_menu_choice_action / 2)))) - battle_soul.y), 6)
        }
        ...
        if (Input_IsPressed(INPUT.CANCEL)) {
            ...
            Anim_Destroy(battle_soul) Anim_Create(battle_soul, "image_angle", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.image_angle, -battle_soul.image_angle, 20) audio_play_sound(snd_menu_confirm, 0, false);
            Battle_EndMenu();
        }
    } else
    ...
    if (_menu == BATTLE_MENU.ITEM) {
        //上/下
        if (battle_soul.image_angle != 90) Anim_Create(battle_soul, "image_angle", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.image_angle, 90 - battle_soul.image_angle, 20) with(text_typer) {
            _surface_target = Battle_GetBoardSurface() if (round(y) != battle_board.y - battle_board.up - 5 + 20 - 32 * battle._menu_choice_item_first) Anim_Create(id, "y", ANIM_TWEEN.CUBIC, ANIM_EASE.OUT, id.y, battle_board.y - battle_board.up - 5 + 20 - 32 * battle._menu_choice_item_first - id.y, 20)
        }
        if (Input_IsPressed(INPUT.UP)) {
        ...
        if (battle_soul.y = battle_button.y + 22) {
            ...
            if (round(battle_soul.y) != ((((battle_board.y - battle_board.up) - 5) + 36) + (32 * (Battle_GetMenuChoiceItem() - _menu_choice_item_first)))) Anim_Create(battle_soul, "y", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.y, (((((battle_board.y - battle_board.up) - 5) + 36) + (32 * (Battle_GetMenuChoiceItem() - _menu_choice_item_first))) - battle_soul.y), 20)
        }
    } else
    ...
    if (_menu == BATTLE_MENU.MERCY) {
        ...
        if (battle_soul.y = battle_button.y + 22) {
            ...
            if (round(battle_soul.y) != ((((battle_board.y - battle_board.up) - 5) + 36) + (32 * _menu_choice_mercy))) Anim_Create(battle_soul, "y", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.y, (((((battle_board.y - battle_board.up) - 5) + 36) + (32 * _menu_choice_mercy)) - battle_soul.y), 20)
        }
        //取消/确定
        if (Input_IsPressed(INPUT.CANCEL)) {
            Battle_SetMenu(BATTLE_MENU.BUTTON);
