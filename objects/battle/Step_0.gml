...
if (!global.classic_ui) {
    if (_state == BATTLE_STATE.MENU) {
        ...
        if (_menu == BATTLE_MENU.FIGHT_TARGET) {
            //上/下
            if (battle_soul.image_angle != 90) Anim_Create(battle_soul, "image_angle", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.image_angle, 90 - battle_soul.image_angle, 20);
            ...
            if (Input_IsPressed(INPUT.CANCEL)) {
                ...
                Anim_Create(battle_soul, "y", 0, 0, battle_soul.y, battle_button.y + 22 - battle_soul.y, 1, , , , 1);
        ...
        if (_menu == BATTLE_MENU.ACT_ACTION) {
            ...
            //返回/确定
            if (Input_IsPressed(INPUT.CANCEL)) {
                Anim_Create(battle_soul, "x", 0, 0, battle_soul.x, ((battle_board.x - battle_board.left) - 5) + 40 - battle_soul.x, 1, , , , 1);
                Anim_Create(battle_soul, "y", 0, 0, battle_soul.y, ((battle_board.y - battle_board.up) - 5) + 36 - battle_soul.y, 1, , , , 1);
                Battle_SetMenu(BATTLE_MENU.ACT_TARGET);
            } else if (Input_IsPressed(INPUT.CONFIRM)) {
                Anim_Destroy(battle_soul);
                Anim_Create(battle_soul, "image_angle", ANIM_TWEEN.CIRC, ANIM_EASE.OUT, battle_soul.image_angle, -battle_soul.image_angle, 20) audio_play_sound(snd_menu_confirm, 0, false);
                Battle_EndMenu();
            }
        } else
