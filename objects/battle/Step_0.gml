...
if(_state==BATTLE_STATE.MENU){
	...
	if(_menu==BATTLE_MENU.FIGHT_TARGET){
		...
		else{  Anim_Create(battle_soul, "y",ANIM_TWEEN.CIRC,ANIM_EASE.OUT, battle_soul.y,(((((battle_board.y - battle_board.up) - 5) + 36) + (32 * _menu_choice_enemy)) - battle_soul.y), 20)}
 ...
 Anim_Create(battle_soul, "x", ANIM_TWEEN.CIRC,ANIM_EASE.OUT, battle_soul.x, (((((battle_board.x - battle_board.left) - 5) + 40) + (256 * (_menu_choice_action % 2))) - battle_soul.x), 6)
	...
	if(_menu==BATTLE_MENU.ITEM){
		//上/下
		Anim_Create(battle_soul,"image_angle",ANIM_TWEEN.CIRC,ANIM_EASE.OUT,battle_soul.image_angle,90-battle_soul.image_angle,20)
		with(text_typer){Anim_Create(id,"y",ANIM_TWEEN.CUBIC,ANIM_EASE.OUT,id.y,battle_board.y-battle_board.up-5+20-32*battle._menu_choice_item_first-id.y,20)}
		if(Input_IsPressed(INPUT.UP)){
	...
	if(battle_soul.y=battle_button.y+22){battle_soul.y=battle_board.y-battle_board.up-5+36+32*(Battle_GetMenuChoiceItem()-_menu_choice_item_first);
	}
		
	
		else{ Anim_Create(battle_soul, "y",ANIM_TWEEN.CIRC,ANIM_EASE.OUT, battle_soul.y, (((((battle_board.y - battle_board.up) - 5) + 36) + (32 * (Battle_GetMenuChoiceItem() - _menu_choice_item_first))) - battle_soul.y), 20)}
	...
	if(_menu==BATTLE_MENU.MERCY){
		//上/下
			Anim_Create(battle_soul,"image_angle",ANIM_TWEEN.QUAD,ANIM_EASE.OUT,battle_soul.image_angle,90-battle_soul.image_angle,20)
		if(Input_IsPressed(INPUT.UP)){
...
if(damage){var _temp_local_var_1,time=0;
