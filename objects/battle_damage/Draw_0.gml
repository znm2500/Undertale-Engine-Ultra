...
if(damage<=0){
	draw_sprite_ext(spr_battle_damage_miss,0,xstart,y,1,yscale,0,c_gray,alpha);
}else{
	...
	var proc=1;
	repeat(LEN){
		draw_sprite_ext(spr_battle_damage,real(string_char_at(STR,proc)),draw_x,y,1,yscale,0,color,alpha);
		draw_x+=SPR_W+1;
		proc+=1;
	}
}