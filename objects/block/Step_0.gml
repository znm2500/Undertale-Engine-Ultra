if(pus_hspeedoulEnabled){
	...
	if(Battle_GetState()==BATTLE_STATE.TURN_PREPARATION||Battle_GetState()==BATTLE_STATE.IN_TURN){
		while(place_meeting(x,y,battle_soul)&&tryTimes>0){
			battle_soul.x+=lengthdir_x(0.1,pus_hspeedoulDirection+image_angle);
			battle_soul.y+=lengthdir_y(0.1,pus_hspeedoulDirection+image_angle);
			tryTimes-=1;
		}
	}
}