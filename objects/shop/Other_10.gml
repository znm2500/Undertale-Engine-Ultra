...
case SHOP_STATE.MENU:
    if (_typer_left_refresh) _typer_left.text = _pre + _host.menu_text;
    if (_typer_right_refresh) _typer_right.text = _pre_inst + "  Buy&  Sell&  Talk&  Exit";
...
case SHOP_STATE.BUY:
    if (instance_exists(_typer_info) && _typer_info_refresh) {
        var item = items.GetOrUndefined(_host.shop_item[_pre_index < 3 ? _pre_index: _exit_index]);
            ...
            if (_typer_left_refresh) {
                ...
                for (i = 0; i < 4; i++) {
                    if (2 * i < NUM) {
                        ...
                        if (MAXLENGTH - LEN > 0) {
                            repeat(MAXLENGTH - LEN) _typer_left.text += " ";
                        }
            ...
            if (_typer_right_refresh) {
                ...
                for (i = 0; i < 4; i++) {
                    if ((2 * i + 1) < NUM) {
                        ...
                        LEN = string_length(string(GOLD));
                        if (MAXLENGTH - LEN > 0) {
                            repeat(MAXLENGTH - LEN) _typer_right.text += " ";
                        }
...
case SHOP_STATE.DIALOG:
    ...
    if (_typer_right_refresh) _typer_right.text = _pre + _host.dialog_before_text;
    break;
default:
    break;
}