if (!battle_soul_aqua.type) {
	...
	if (STATE == BATTLE_STATE.TURN_PREPARATION || STATE == BATTLE_STATE.IN_TURN) {
		...
		for (var i = 0; i < boardcount; i++) { //遍历所有框,判断是否出框
			...
			isContains[2] = global.boards_array[i].contains(x, y - battle_soul.sprite_height / 2);
			isContains[3] = global.boards_array[i].contains(x, y + battle_soul.sprite_height / 2);
			if (isContains[0]) {
		...
		if ((isInside[2][0] || isInside[2][1] || isInside[2][2] || isInside[2][3]) && (isInside[1][0] && isInside[1][1] && isInside[1][2] && isInside[1][3])) {
			...
			if (!isInside[2][3]) {
				var nearestPos = [x, y + battle_soul.sprite_height / 2],
