...
if (_char != " " && _char != "　") {
   ...
   var INST = instance_create_depth(
    x + lengthdir_x(
        point_distance(0, 0, _char_x, _char_y + OFFSET),
        point_direction(0, 0, _char_x, _char_y + OFFSET) + _angle
    ),
    y + lengthdir_y(
        point_distance(0, 0, _char_x, _char_y + OFFSET),
        point_direction(0, 0, _char_x, _char_y + OFFSET) + _angle
    ...
    if (sprite_exists(_char_sprite)) {
       ...
       INST.x += lengthdir_x(
    point_distance(0, 0, sprite_get_xoffset(_char_sprite) * _scale_x, sprite_get_yoffset(_char_sprite) * _scale_y),
    point_direction(0, 0, sprite_get_xoffset(_char_sprite) * _scale_x, sprite_get_yoffset(_char_sprite) * _scale_y) + _angle
);
INST.y += lengthdir_y(
    point_distance(0, 0, sprite_get_xoffset(_char_sprite) * _scale_x, sprite_get_yoffset(_char_sprite) * _scale_y),
    point_direction(0, 0, sprite_get_xoffset(_char_sprite) * _scale_x, sprite_get_yoffset(_char_sprite) * _scale_y) + _angle
  ...
  var INST_OFFSET = new Vector3(
    sprite_exists(_char_sprite) ? sprite_get_xoffset(_char_sprite) * _scale_x : 0 + _char_x,
    sprite_exists(_char_sprite) ? sprite_get_yoffset(_char_sprite) * _scale_y : 0 + _char_y + OFFSET
);
    ds_list_add(_list_inst, INST);
    ds_list_add(_list_inst_offset, INST_OFFSET);
