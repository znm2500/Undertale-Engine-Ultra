...
if (instance_exists(_inst_name)) _inst_name.x = _offset + 23
...
case 0:
    if (Input_IsPressed(INPUT.UP)) {
        ...
        case 0:
            if (Item_GetNumber() > 0) {
                _menu = 1;
                index = 0 Anim_Create(_show_width, string(index), ANIM_TWEEN.CUBIC, ANIM_EASE.IN, _show_width[index], 176 - _show_width[index], 20);
                event_user(0);
        ...
        case 1:
            _menu = 3;
            index = 1 Anim_Create(_show_width, string(index), ANIM_TWEEN.CUBIC, ANIM_EASE.IN, _show_width[index], 176 - _show_width[index], 20);
            event_user(0);
            audio_play_sound(snd_menu_confirm, 0, false);
            break;
        case 2:
            _menu = 4;
            index = 2 Anim_Create(_show_width, string(index), ANIM_TWEEN.CUBIC, ANIM_EASE.IN, _show_width[index], 176 - _show_width[index], 20);
            event_user(0);
            audio_play_sound(snd_menu_confirm, 0, false);
            break;
        }
...
case 3:
    if (Input_IsPressed(INPUT.CANCEL)) {
        Anim_Create(id._show_width, "1", ANIM_TWEEN.CUBIC, ANIM_EASE.OUT, _show_width[1], 0 - _show_width[1], 20);
        _menu = 0;
        event_user(0);
    }
    break;
case 4:
    if (Input_IsPressed(INPUT.UP)) {
        ...
        _menu = -1;
        Anim_Create(_show_width, "2", ANIM_TWEEN.CUBIC, ANIM_EASE.OUT, _show_width[2], 0 - _show_width[2], 20);
        Anim_Create(id, "_offset", ANIM_TWEEN.CUBIC, ANIM_EASE.IN, _offset, -100 - _offset, 15);
        event_user(0);
        audio_play_sound(snd_menu_confirm, 0, false);
    } else if (Input_IsPressed(INPUT.CANCEL)) {
