    ...
    if (Input_IsPressed(INPUT.UP)) {
        ...
        case 0:
            if (Item_GetNumber() > 0) {
                _menu = 1;
                index = 0;
                Anim_Create(_show_width, string(index), ANIM_TWEEN.CUBIC, ANIM_EASE.IN, _show_width[index], 176 - _show_width[index], 20);
                if (global.classic_ui) Anim_Skip(_show_width, string(index));
                event_user(0);
        ...
        alarm[0] = global.classic_ui ? 1 : 15 / global.delta_time_factor;
        _menu = -1;
...
case 1:
    if (Input_IsPressed(INPUT.UP)) {
        ...
        Anim_Create(_show_width, "0", ANIM_TWEEN.CUBIC, ANIM_EASE.OUT, _show_width[0], 0 - _show_width[0], 20);
        if (global.classic_ui) Anim_Skip(_show_width, "0");
        _menu = 0;
        event_user(0);
    }
    break;
case 2:
    if (Input_IsPressed(INPUT.LEFT)) {
        if (_choice_item_operate > 0) {
            ...
            if (global.classic_ui) Anim_Skip(id, "_choice_item_operate_soul");
        }
        ...
        alarm[0] = global.classic_ui ? 1 : 15 / global.delta_time_factor;
        event_user(0);
        Anim_Create(_show_width, "0", ANIM_TWEEN.CUBIC, ANIM_EASE.OUT, _show_width[0], 0 - _show_width[0], 15);
        if (global.classic_ui) Anim_Skip(_show_width, "0");
        Anim_Create(id, "_offset", ANIM_TWEEN.CUBIC, ANIM_EASE.IN, _offset, -200 - _offset, 15);
        if (global.classic_ui) Anim_Skip(id, "_offset");
        
        var items = Item_GetInventoryItems();
        switch (_choice_item_operate) {
        case 0: items.InvokeItemUse(_choice_item); break;
        case 1: items.InvokeItemInfo(_choice_item); break;
        case 2: items.InvokeItemDrop(_choice_item); break;
        }
        audio_play_sound(snd_menu_confirm, 0, false);
    } else if (Input_IsPressed(INPUT.CANCEL)) {
        _menu = 1;
...
case 3:
    if (Input_IsPressed(INPUT.CANCEL)) {
        Anim_Create(_show_width, "1", ANIM_TWEEN.CUBIC, ANIM_EASE.OUT, _show_width[1], 0 - _show_width[1], 15);
        if (global.classic_ui) Anim_Skip(_show_width, "1");
        _menu = 0;
        event_user(0);
    }
    break;
case 4:
    if (Input_IsPressed(INPUT.UP)) {
        if (_choice_phone > 0) {
            _choice_phone -= 1;
            Anim_Create(id, "_choice_phone_soul", ANIM_TWEEN.CUBIC, ANIM_EASE.OUT, _choice_phone_soul, _choice_phone - _choice_phone_soul, 15);
            if (global.classic_ui) Anim_Skip(id, "_choice_phone_soul");
            audio_play_sound(snd_menu_switch, 0, false);
        }
    } else if (Input_IsPressed(INPUT.DOWN)) {
        ...
        _menu = -1;
        alarm[0] = global.classic_ui ? 1 : 15 / global.delta_time_factor;
        Anim_Create(_show_width, "2", ANIM_TWEEN.CUBIC, ANIM_EASE.OUT, _show_width[2], 0 - _show_width[2], 15);
        if (global.classic_ui) Anim_Skip(_show_width, "2");
        Anim_Create(id, "_offset", ANIM_TWEEN.CUBIC, ANIM_EASE.IN, _offset, -200 - _offset, 15);
        if (global.classic_ui) Anim_Skip(id, "_offset");
