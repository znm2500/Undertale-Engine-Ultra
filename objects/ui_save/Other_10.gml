if(_state==0){
	...
	_inst_name=instance_create_depth((108+6+26)/2,(118+6+16)/2,0,text_typer);
	_inst_name.text=_prefix+Flag_Get(FLAG_TYPE.INFO,FLAG_INFO.NAME,"EMPTY");
	
	_inst_lv=instance_create_depth((108+6+180)/2,(118+6+16)/2,0,text_typer);
	_inst_lv.text=_prefix+"LV "+string(Flag_Get(FLAG_TYPE.INFO,FLAG_INFO.LV));
	
	_inst_time=instance_create_depth((108+6+338)/2,(118+6+16)/2,0,text_typer);
	...
	_inst_room=instance_create_depth((108+6+26)/2,(118+6+56)/2,0,text_typer);
	_inst_room.text=_prefix+Player_GetRoomName(Flag_Get(FLAG_TYPE.INFO,FLAG_INFO.ROOM,-1));
	
	_inst_save=instance_create_depth((108+6+56)/2,(118+6+116)/2,0,text_typer);
	_inst_save.text=_prefix+"Save";
	
	_inst_return=instance_create_depth((108+6+236)/2,(118+6+116)/2,0,text_typer);
