if (_state == 0) {
    ...
    _inst_name.text = _prefix + "{surface " + string(real(_surface_text)) + "}" + Flag_Get(FLAG_TYPE.INFO, FLAG_INFO.NAME, "EMPTY");

    _inst_lv = instance_create_depth((108 + 6 + 180), (118 + 6 + 16), 0, text_typer);
    _inst_lv.text = _prefix + "{surface " + string(real(_surface_text)) + "}" + "LV " + string(Flag_Get(FLAG_TYPE.INFO, FLAG_INFO.LV));
    ...
    _inst_room.text = _prefix + "{surface " + string(real(_surface_text)) + "}" + Player_GetRoomName(Flag_Get(FLAG_TYPE.INFO, FLAG_INFO.ROOM, -1));
