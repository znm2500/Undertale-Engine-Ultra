...
function Anim_Uninit() {
	if (is_array(global._anim_data)) {
		var i = 0;
		repeat(array_length(global._anim_data)) {
			delete(global._anim_data[i++]);
		}
	...
	for (var i = 0; i < array_length(ease_list); i++) {
		...
		if (anim_item[$ "time"] - anim_item[$ "delay"] >= 0) {
			...
			switch (anim_item[$ "target_type"]) {
				...
				case 3: {
					with(anim_item[$ "target"])
					variable_instance_set(id, anim_item[$ "var_name"], anim_value * anim_item[$ "change"] + anim_item[$ "tweenstart"]);
					break;
				}
				case 4: {
